...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  let rdvs = db.rdvs;
  if (evenementId) rdvs = rdvs.filter((r) => r.evenementId === Number(evenementId));
  if (visiteurId)  rdvs = rdvs.filter((r) => r.visiteurId  === Number(visiteurId));
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (!body.visiteurId || !body.objetId || !body.evenementId) {
    return NextResponse.json({ error: "visiteurId, objetId et evenementId sont requis" }, { status: 400 });
  }

  // Validate visiteur exists
  const visiteur = db.visiteurs.find((v) => v.id === body.visiteurId);
  if (!visiteur) {
    return NextResponse.json({ error: "Visiteur introuvable" }, { status: 400 });
  }

  // Validate objet belongs to the visiteur
  const objet = visiteur.objets.find((o) => o.id === body.objetId);
  if (!objet) {
    return NextResponse.json({ error: "Objet introuvable pour ce visiteur" }, { status: 400 });
  }
